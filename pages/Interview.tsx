...
import { useTurnBasedAgent } from '../hooks/useTurnBasedAgent';
...
// Shared UI Component that handles display and common logic
const InterviewLayout = ({ 
    ...
    const { isConnected, isSpeaking, connect, disconnect, volume } = agent;
    const { t } = useLanguage();
...
const appendQuestionsToInstruction = (plan: ResearchPlan) => {
    let instruction = plan.systemInstruction;
    if (plan.questions && plan.questions.length > 0) {
        instruction += `\n\n[MANDATORY INTERVIEW QUESTIONS]\nYou must ask the following questions in the exact order listed below. Do not skip any questions. Do not make up your own questions unless asking for clarification.\n\n`;
        instruction += plan.questions.map((q, i) => `Question ${i + 1}: ${q.text} (Intent: ${q.intent})`).join('\n');
        instruction += `\n\n[END OF QUESTIONS]\nWhen all questions are answered, say "Thank you, the interview is over" or "访谈结束".`;
    }
    return instruction;
};

const LiveInterviewSession = ({ plan, onFinish, onTranscriptUpdate, transcript }: any) => {
    const { language } = useLanguage();
    const fullInstruction = appendQuestionsToInstruction(plan);
    ...
    return <InterviewLayout agent={agent} transcript={transcript} onFinish={onFinish} language={language} />;
};

const TurnBasedInterviewSession = ({ plan, onFinish, onTranscriptUpdate, transcript }: any) => {
    const { language } = useLanguage();
    const fullInstruction = appendQuestionsToInstruction(plan);

    const agent = useTurnBasedAgent({
        systemInstruction: fullInstruction,
...
export const Interview: React.FC<InterviewProps> = ({ plan, onFinish }) => {
  ...
  const isGeminiLive = !plan.provider || plan.provider === 'gemini';
